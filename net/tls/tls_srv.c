...
bad_version:
	...
	T_FSM_STATE(TTLS_CH_HS_RND) {
		...
		memcpy(tls->hs->randbytes + io->rlen, p, n);
		p += n;
		io->hslen -= n;
	...
	T_FSM_STATE(TTLS_CH_HS_SESS) {
		...
		memcpy(tls->sess.id + io->rlen, p, n);
		p += n;
		io->hslen -= n;
	...
	TTLS_HS_FSM_FINISH();
	/*
	 * Return from the function if we need more data (r == -EAGAIN,
	 * we'll reenter the function for further processing) or a failure
	 * (all other codes except 0).
	 */
	if (r != 0)
		return r;
...
ttls_write_alpn_ext(TlsCtx *tls, unsigned char *p, size_t *olen)
{
	...
	*olen = 7 + tls->alpn_chosen->len;
	...
	memcpy(p + 7, tls->alpn_chosen->name, *olen - 7);
}
			...
			unsigned char **in_buf)
{
	...
	*p++ = (unsigned char)tls->sess.id_len;
	memcpy(p, tls->sess.id, tls->sess.id_len);
	p += tls->sess.id_len;
			       ...
			       size_t hh_len, unsigned int *read)
{
	...
	if (io->rlen + len < io->hslen) {
		if (unlikely(io->hslen > TTLS_HS_RBUF_SZ))
			return TTLS_ERR_BAD_HS_CLIENT_KEY_EXCHANGE;
		memcpy(&tls->hs->key_exchange_tmp[io->rlen], buf, len);
		*read += len;
		io->rlen += len;
		return -EAGAIN;
	}
...
ttls_handshake_init_out_buffers(TlsCtx *tls, struct sg_table *sgt,
				unsigned char **p, struct page **pg)
{
	/*
			   ...
			   size_t hh_len, unsigned int *read)
{
	 ...
	 */
	T_FSM_STATE(TTLS_SERVER_HELLO) {
		if ((r = ttls_handshake_init_out_buffers(tls, &sgt, &p, &pg)))
			T_FSM_EXIT();
