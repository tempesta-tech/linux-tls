...
ttls_mpi_copy_alloc(TlsMpi *X, const TlsMpi *Y, bool need_alloc)
{
	...
	X->s = Y->s;
	X->used = Y->used;
	memset(MPI_P(X) + X->used, 0, (X->limbs - X->used) * CIL);
}
...
ttls_mpi_lset(TlsMpi *X, long z)
{
	...
	memset(MPI_P(X) + 1, 0, (X->limbs - 1) * CIL);
}
...
zero_sign:
	if (X->used == 1 && !x[0])
		X->s = 1;
	memset(x + X->used, 0, (X->limbs - X->used) * CIL);
}
...
ttls_mpi_add_abs(TlsMpi *X, const TlsMpi *A, const TlsMpi *B)
{
	...
	BUG_ON(r <= 0);
	X->used = r;
	memset(MPI_P(X) + r, 0, (X->limbs - r) * CIL);
		 ...
		 TlsMpi *RR)
{
	...
	memset(W, 0, sizeof(TlsMpi) * (1 << MPI_W_SZ));
