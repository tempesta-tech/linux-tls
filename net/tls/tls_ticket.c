...
ttls_ticket_get_time(unsigned long lifetime)
{
	...
	return ts.tv_sec - (ts.tv_sec % lifetime);
}
...
ttls_ticket_update_keys(TlsTicketPeerCfg *tcfg)
{
	...
	if (likely(tcfg->keys[tcfg->active_key].ts < ts) || !ts) {
		...
		old_key->ts = ts;
		memcpy(old_key->key, digest, sizeof(old_key->key));
...
ttls_tickets_key_search_locked(TlsTicketPeerCfg *tcfg, const char *key_name)
{
	...
	key = &tcfg->keys[tcfg->active_key];
	read_lock(&key->lock);
	r = memcmp(key_name, key->name, TTLS_TICKET_KEY_NAME_LEN);
	if (!r && key->ts)
		goto found;
...
ttls_ticket_sess_save(const TlsSess *sess, TlsState *state, size_t buf_len)
{
	...
	memcpy(&state->sess, sess, sizeof(TlsSess));
	state->cert_len = sess->peer_cert ? sess->peer_cert->raw.len : 0;

	if (sess->peer_cert)
		memcpy(state->cert_data, sess->peer_cert->raw.p,
			    state->cert_len);
...
ttls_ticket_sess_load(TlsState *state, size_t len, unsigned long lifetime)
{
	...
	if (!state->cert_len) {
		...
		memcpy(page_address(pg), state->cert_data, state->cert_len);
...
ttls_ticket_encrypt(TlsTicket *tik, TlsTicketKey *key, size_t full_enc_len)
{
	...
	memcpy(tik->name, key->name, sizeof(key->name));
...
ttls_ticket_parse(TlsCtx *ctx, unsigned char *buf, size_t len)
{
	...
	memcpy(&recv_sess->id, ctx->sess.id, recv_sess->id_len);
	memcpy(&ctx->sess, recv_sess, sizeof(TlsSess));
