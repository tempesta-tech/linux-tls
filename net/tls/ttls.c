...
/**
 * TLS handshakes might be not the only users of sk_user_data, so the
 * handshakes context can be stored by a registered offset letting other
 * users to be placed at the beggining of sk_user_data.
 */
#define tls_sk_ctx(sk)	((TlsCtx *)((char *)(sk)->sk_user_data		\
				    + tls_sk_off_data))
		...
		unsigned char *dstbuf, size_t dlen)
{
	...
	memcpy(tmp + md_len, label, llen);
	memcpy(tmp + md_len + llen, random, rlen);
	llen += rlen;
	...
	for (i = 0; i < dlen; i += md_len) {
		...
		memcpy(dstbuf + i, h_i, k);
	}

	ttls_md_free(&md_ctx);
	memset(__buf, 0, TTLS_MD_MAX_SIZE * 3);
...
ttls_update_checksum(TlsCtx *tls, const unsigned char *buf, size_t len)
{
	...
	if (unlikely(IS_ERR_OR_NULL(ci))) {
			     ...
			     && mac == TTLS_MD_SHA256))
		{
			...
			memcpy(&tls->hs->fin_sha256, sha256, sizeof(*sha256));
		}
	}
...
ttls_calc_verify_tls_sha256(TlsCtx *tls, unsigned char hash[32])
{
	...
	memcpy(&sha256, &tls->hs->fin_sha256, sizeof(sha256));
...
ttls_calc_verify_tls_sha384(TlsCtx *tls, unsigned char hash[48])
{
	...
	memcpy(&sha512, &tls->hs->fin_sha512, sizeof(sha512));
...
ttls_derive_keys(TlsCtx *tls)
{
	...
	memcpy(tmp, hs->randbytes, 32);
	memcpy(hs->randbytes, hs->randbytes + 32, 32);
	...
	memset(hs->randbytes, 0, sizeof(hs->randbytes));
	...
	if (tls->conf->endpoint == TTLS_IS_CLIENT) {
		...
		memcpy(xfrm->iv_enc, key2 + xfrm->keylen, iv_copy_len);
		memcpy(xfrm->iv_dec, key2 + xfrm->keylen + iv_copy_len,
			    iv_copy_len);
	} else {
		...
		mac_dec = keyblk;
		iv_copy_len = xfrm->fixed_ivlen ? : xfrm->ivlen;
		memcpy(xfrm->iv_dec, key1 + xfrm->keylen, iv_copy_len);
		memcpy(xfrm->iv_enc, key1 + xfrm->keylen + iv_copy_len,
			    iv_copy_len);
	}
...
ttls_aead_req_free(struct crypto_aead *tfm, struct aead_request *req)
{
	...
	if (WARN_ON_ONCE(ttls_aead_reqsize() < need))
		...
		memset(req, 0, ttls_aead_reqsize());
}
 ...
 */
static int
ttls_do_encrypt(TlsCtx *tls, struct sg_table *sgt, struct sg_table *out_sgt)
{
	int r, elen;
	TlsXfrm *xfrm = &tls->xfrm;
 ...
 */
static void
tls_tcp_propagate_dseq(struct sock *sk, struct sk_buff *skb)
{
	struct sk_buff *next;
 ...
 */
static int
tls_tcp_add_overhead(struct sock *sk, unsigned int overhead)
{
 ...
 */
static int
skb_to_sgvec_with_new_pages(struct sk_buff *skb, struct scatterlist *sgl,
 ...
 */
int
tls_encrypt(struct sock *sk, struct sk_buff *skb, unsigned int limit)
{
	/*
...
#define AUTO_SEGS_N	8
	...
	T_DBG3("%s: sk=%pK(snd_una=%u snd_nxt=%u limit=%u)"
	       ...
	       skb, skb->len, skb->data_len, tls_skb_type(skb),
	...
	while (!tcp_skb_is_last(sk, skb_tail)) {
		 ...
		 */
		tls_tcp_propagate_dseq(sk, skb_tail);
	...
	if (skb_tail == skb) {
		// TODO see how the things are done in kTLS
	 ...
	 */
	if (tls_tcp_add_overhead(sk, t_sz))
		return -ENOMEM;
	...
	for (next = skb, frags = 0, out_frags = 0; ; ) {
		...
		r = skb_to_sgvec_with_new_pages(next, out_sgt.sgl + out_frags,
						&pages_end);
		if (r <= 0)
			goto out;
		out_frags += r;
...
__ttls_decrypt(TlsCtx *tls, unsigned char *buf)
{
	...
	memcpy(xfrm->iv_dec + xfrm->fixed_ivlen, io->iv, sizeof(io->iv));
		  ...
		  unsigned char *hdr_buf)
{
	 ...
	 * Otherwise tls_encrypt() -> ttls_aad2hdriv(), called from
...
tls_send(TlsCtx *tls, struct sg_table *sgt)
{
	 ...
	 * in tls_encrypt(), so if we have an encryption context, then we
 ...
 * bad, and -EAGAIN if we need more data for the header.
 */
static int
		      ...
		      unsigned int *read)
{
	...
	if (io->hdr_cpsz < TLS_HEADER_SIZE) {
		...
		memcpy(io->hdr + io->hdr_cpsz, buf, n);
		*read += n;
		io->hdr_cpsz += n;
	}
	...
	if (unlikely(io->hdr_cpsz + len < TLS_HEADER_SIZE + ivahs_len)) {
		memcpy(io->__msg + io->hdr_cpsz - TLS_HEADER_SIZE, buf + n, len);
		*read += len;
		io->hdr_cpsz += len;
		return -EAGAIN;
	}
...
static void
ttls_handshake_free(tls_handshake_t *hs)
{
	if (unlikely(!hs))
		return;
	...
	memset(hs, 0, sizeof(tls_handshake_t));
	free_pages((unsigned long)hs, 0);
}
...
ttls_write_finished(TlsCtx *tls, struct sg_table *sgt, unsigned char **in_buf)
{
	...
	if ((r = tls_do_encrypt(tls, &enc_sgt, &enc_sgt)))
		return r;
		    ...
		    unsigned int *read)
{
	...
	memcpy(hs->finished + io->rlen, buf, n);
	io->rlen += n;
	*read += n;
...
ttls_ctx_init(TlsCtx *tls, struct sock *sk, const TlsCfg *conf)
{
	...
	BUILD_BUG_ON(sizeof(tls_handshake_t) > PAGE_SIZE);
	tls->hs = (tls_handshake_t *)__get_free_pages(GFP_ATOMIC | __GFP_ZERO, 0);
	if (!tls->hs)
		return -ENOMEM;
...
ttls_set_hostname(TlsCtx *tls, const char *hostname)
{
	 ...
	 */
	if (tls->hostname) {
		memset(tls->hostname, 0, strlen(tls->hostname));
		kfree(tls->hostname);
	}
 ...
 * TTLS_HANDSHAKE_OVER.
 *
 * The step callers must return -EAGAIN if more input data is required to
 * completely read current ingress record and 0 (0) if current FSM state
 ...
 *
 * @return -EAGAIN if there is no ready data for upper layer (e.g. during
...
next_msg:
	...
	case -EPROTO:
		spin_unlock(&tls->lock);
		if (!ttls_hs_done(tls) && ttls_hs_over_cb)
			ttls_hs_over_cb(tls, TTLS_HS_CB_INCOMPLETE);
	...
	if (tls->io_in.msgtype == TTLS_MSG_APPLICATION_DATA) {
		 ...
		 */
		r = tls_chop_skb_rec(tls, tls->io_in.skb_list, &dec_app_data);
		if (r) {
			spin_unlock(&tls->lock);
			return r;
		}
 ...
 *
 * @sk_off_udata	- offset from sk->sk_user_data to get TlsCtx.
 *
 * @ttls_send_cb	- tls_sock_send() by default. The callback may accept
 ...
 *			  tickets.
 *
 * XXX Should we make the offset and callbacks a socket or module specific for
 * several TLS users (e.g. an RPC-over-TLS plus to generic TLS acceleration
 * exported to the user space)?
 */
void
ttls_register_callbacks(size_t sk_off_udata, ttls_send_cb_t *send_cb,
			ttls_sni_cb_t *sni_cb, ttls_hs_over_cb_t *hs_over_cb,
			ttls_cli_id_t *cli_id_cb)
{
	tls_sk_off_data = sk_off_udata;
		 ...
		 size_t len)
{
	...
	return !memcmp(proto->name, buf, len);
}

#if defined(DBG_TLS) && DBG_TLS >= 3
unsigned long
