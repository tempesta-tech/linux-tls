 ...
 *
 * TODO #1335: replace the implementation with the natice Linux one.
 *
...
ecp_mod_p255(TlsMpi *N)
{
	...
	if (M->used > G_LIMBS + 1)
		M->used = G_LIMBS + 1;
	n = M->used * CIL;
	memcpy(MPI_P(M), MPI_P(N) + G_LIMBS - 1, n);
	memset((char *)MPI_P(M) + n, 0, (G_LIMBS + 2) * CIL - n);
