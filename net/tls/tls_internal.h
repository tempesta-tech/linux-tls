...
#define BANNER	"tls handshake: "

#define T_ERR(...)	net_err_ratelimited(BANNER "ERROR: " __VA_ARGS__)
#define T_WARN(...)	net_warn_ratelimited(BANNER "Warning: "	__VA_ARGS__)
#define T_LOG(...)	net_info_ratelimited(BANNER __VA_ARGS__)

/* Non-limited printing. */
#define T_ERR_NL(...)	pr_err(BANNER __VA_ARGS__)
#define T_WARN_NL(...)	pr_warn(BANNER __VA_ARGS__)
#define T_LOG_NL(...)	pr_log(BANNER __VA_ARGS__)

#ifdef DBG_TLS
#define T_DBG(...)	pr_debug(BANNER " " __VA_ARGS__)

#if DBG_TLS >= 2
#define T_DBG2(...)	pr_debug(BANNER "   " __VA_ARGS__)
#else /* TLS_DBG < 2 */
#define T_DBG2(...)
#endif

#if DBG_TLS == 3
#define T_DBG3(...)	pr_debug(BANNER "     " __VA_ARGS__)
#define T_DBG3_BUF(str, buf, len)					\
do {									\
	T_DBG3("dump '" str "' (ptr=%pk, %lu bytes):\n",		\
	       buf, (size_t)(len));					\
	print_hex_dump_bytes(BANNER "        ", DUMP_PREFIX_OFFSET, buf, len);\
} while (0)
